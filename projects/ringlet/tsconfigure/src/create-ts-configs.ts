...
import { PackageConfig, Project } from '@thrashplay/project-utils'

import { BuildConfiguration, PackageBuildStep } from './cli/steps/build-step'
 ...
 * extend that. If there is no such file, then the extends path will extend the monorepo project root's Ringlet 
 * tsconfig.json.
 * 
 * TODO: For a standalone project, TBD
 */
const getExtendsPathForPackage = (project: Project, packageConfig: PackageConfig) => {
  const packageRingletDir = path.join(packageConfig.directory, '.ringlet')
  const projectRingletDir = path.join(project.directory, '.ringlet')
  const packageRelativeDir = path.relative(packageRingletDir, packageConfig.directory)
  const projectRelativeDir = path.relative(packageRingletDir, projectRingletDir)
...
const getReferencesForPackage = (project: Project, packageConfig: PackageConfig) => {
  ...
  const createReferenceForPackage = (otherPackage: PackageConfig) => {
    const currentConfigDirectory = path.join(packageConfig.directory, '.ringlet')
    const dependencyConfigDirectory = path.join(otherPackage.directory, '.ringlet')
    return {
...
class CreatePackageTsConfigBuildStep extends PackageBuildStep {
  protected beforePackages = (_configuration: BuildConfiguration, project: Project) => {
    return project.readJsonFile('tsconfig.json')
      ...
      .then((tsconfig) => {
        if (!isNil(tsconfig)) {
          this.assertNoForbiddenProperties(path.resolve(project.directory, 'tsconfig.json'), tsconfig)
        }
        ...
        return fs.mkdir(path.resolve(project.directory, '.ringlet'), { recursive: true })
          .then(() => Promise.all(concat(
            project.writeJsonFile(path.join('.ringlet', 'tsconfig.json'), config),
          )))
          .then(() => true)
      })
  }
  ...
  protected executeForPackage(_configuration: BuildConfiguration, project: Project, packageConfig: PackageConfig) {
    const assertExtendsIsValid = (tsconfigPath: string, tsconfig: object) => {
      const packageTsconfigDirectory = path.resolve(packageConfig.directory, '.ringlet')
      const rootTsconfigFile = path.resolve(project.directory, '.ringlet', 'tsconfig.json')
      ...
      if (get(tsconfig, 'extends') !== requiredValue) {
        throw new Error(`Error in ${tsconfigPath}: The 'extends' option must be set, and must reference the Ringlet-generated root configuration: ${requiredValue}.`)
      }
    }
    ...
    return packageConfig.readJsonFile('tsconfig.json')
      ...
      .then((tsconfig) => {
        ...
        return packageConfig.writeJsonFile(path.join('.ringlet', 'tsconfig.json'), packageTsConfig)
      })
  }
  ...
  private assertNoForbiddenProperties = (tsconfigPath: string, tsconfig: object) => {
    const assertCompilerOptionNotSet = (userBaseTsConfig: object, property: string) => {
      if (has(userBaseTsConfig, property)) {
        throw new Error(`Error in ${tsconfigPath}: The tsconfig.json option '${property}' cannot be set, because it will be overridden by Ringlet.`)
      }
    }
