...
const createTests = (fileSystemInitializer: () => string) => () => {
  ...
  describe('when package.json IS valid', () => {
    it('loads package.json', async () => {
      const packageJson = await createPackageJson(directory)
      expect(packageJson).toMatchObject(fixtures.packageJson.default)
    })
  })
  ...
  describe('when package.json IS NOT valid', () => {
    it('throws error, when NO package.json', async () => {
      ...
      await expect(createPackageJson(directory)).rejects.toThrowError()
    })
    ...
    it('throws error, when package.json is not valid JSON', async () => {
      ...
      await expect(createPackageJson(directory)).rejects.toThrow(`Invalid package.json format: ${packageJsonPath}`)
    })
    ...
    it('throws error, when package.json has invalid structure', async () => {
      ...
      return expect(createPackageJson(directory))
