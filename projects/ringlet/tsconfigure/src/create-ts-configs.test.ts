...
import { create } from './create-ts-configs'
...
const createMonorepoConfigTests = (cwd: string, projectRootDir: string) => () => {
  ...
  const project = new Project(true, projectRootDir, packageConfigs)

  describe.only('root .ringlet/tsconfig.json file', () => {
    const rootTsConfig = fixtures.tsconfig.default
    const rootTsConfigString = JSON.stringify(rootTsConfig, null, 2)

    it.only('extends existing project-level tsconfig.json, when one exists', async () => {
      ...
      assertJsonFile(path.resolve(projectRootDir, '.ringlet', 'tsconfig.json'), (contents) => {
  ...
  describe('package-level .thrasher/tsconfig.json files', () => {
    ...
    it.each<[string, PackageConfig, object[]]>([
      [
...
const createStandaloneProjectConfigTests = (cwd: string, projectRootDir: string) => () => {
  ...
  const project = new Project(false, projectRootDir, [packageConfig])
