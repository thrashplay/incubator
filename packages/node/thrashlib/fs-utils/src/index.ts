...
/**
 * Returns true if the given path exists, or false otherwise.
 * 
 * If provided, the second argument is a function to check the `fs.Stats` object. If that
 * function is not specified or returns true for an existing path, this function will return 
 * true. If the function returns false, then this function will return false.
 ...
 * @param path the path to check
 */
export const pathExistsSync = (path: string, predicate?: (stats: Stats) => boolean) => {
  try {
    const stats = statSync(path)
    return isNil(predicate) || predicate(stats)
  } catch (error) {
    // if stat called failed because file doesn't exist, just return false
    if (error.code === 'ENOENT') {
      return false
...
/**
 * Returns `true` if the given path exists and is a regular file or false otherwise.
 * @param path the path to check
 */
export const fileExistsSync = (path: string) => pathExistsSync(path, (stats) => stats.isFile())
...
/**
 * Returns `true` if the given path exists and is a directory or false otherwise.
 * @param path the path to check
 */
export const directoryExistsSync = (path: string) => pathExistsSync(path, (stats) => stats.isDirectory())